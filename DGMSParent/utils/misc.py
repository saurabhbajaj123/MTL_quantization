...
from DGMSParent.utils.cluster import kmeans
from DGMSParent.utils.lr_scheduler import get_scheduler
...
def cluster_weights(weights, n_clusters):
    ...
    # _cluster_idx, region_saliency = kmeans(X=flat_weight, num_clusters=n_clusters, tol=_tol, \
    #                     distance='euclidean', device=torch.device('cpu'), tqdm_flag=False)
    # pi_initialization = torch.tensor([torch.true_divide(_cluster_idx.eq(i).sum(), _cluster_idx.numel()) \
    #                         for i in range(n_clusters)], device='cpu')
    ...
    # sigma_initialization = torch.tensor([torch.true_divide(sigma_tmp[i], _cluster_idx.eq(i).sum()-1) \
                                    # for i in range(n_clusters)], device='cpu').sqrt()
    ...
    # sigma_initialization = sigma_initialization[torch.arange(region_saliency.size(0)) != zero_center_idx]    
    
    pi_initialization = pi_initialization[torch.arange(region_saliency.size(0)).cuda() != zero_center_idx]
    # pi_initialization = pi_initialization[torch.arange(region_saliency.size(0)) != zero_center_idx]

    region_saliency = region_saliency[torch.arange(region_saliency.size(0)).cuda() != zero_center_idx] # remove zero component center
    # region_saliency = region_saliency[torch.arange(region_saliency.size(0)) != zero_center_idx] # remove zero component center
